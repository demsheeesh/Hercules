...
void pc_calc_ranking(struct map_session_data *tsd, struct map_session_data *ssd, int skill_id)
{
	...
	if (map_allowed_woe(m))
	{
		...
		*------------------------------------------*/
		...
		//Target msg
		sprintf(output, "( The %s [%s] kill you using <%s> )", pc->job_name(ssd->status.class), ssd->status.name, (skill_id ? skill->get_desc(skill_id) : "Melee/Reflect/Effect"));
 ...
 *
 **/
static int pc_dead(struct map_session_data *sd,struct block_list *src, int skill_id)
{
	nullpo_ret(sd);
